...
class jsado:
	...
	def __getHackString(self,f, l):
		...
		$function = function(code) {
			...
			//get in output
			$cont++;
			if(window.$code == undefined || window.$code.$cont == undefined)$str += input +"<br/><br/>";
	...
	def applyHack(self, fileInput, functionName, limitExecution, useJB):
		try:
			...
			injString = ""	
			if useJB == True:
				injString += "<script src=\"beautify.js\"></script>" #you should download js lib from https://github.com/einars/js-beautify
...
def runDeobfuscation(inputFileName, hackFunction, execLimit, useJsBeauty, injStart):
    
    global browserName, outputFileName, useSelenium
    ...
    try:	
        ...
        if(hack.applyHack(inputFileName, hackFunction, execLimit, useJsBeauty) == 0):
...
if __name__ == "__main__":
	...
	runDeobfuscation(sys.argv[1], sys.argv[2], execLimit, useJsBeauty, injStart)
